...
=======
    if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_FILES['file1']) && isset($_FILES['file2'])) {
        ...
        //generer tableau de surveillant
        foreach ($pv as $filiere => $matieres) {
            foreach ($matieres as $matiere => $salles) {
                foreach ($salles as $salle => $info) {  
                    $pdf->AddPage();  
                     // Générer le tableau HTML avec deux colonnes
                    $html = '<table  border="0" cellpadding="5" cellspacing="5" style="width:100%;">
                                    <tr>
                                        <td style="width:50%;">' . genererTabHeader($salles) . '</td>
                                    </tr>
                                  </table><br><br>';
                    foreach ($filieres as $f => $etudiants) {
                        // Diviser les étudiants en deux colonnes
                        ...
                        $nomCompletFiliere = isset($nomsFilieres[$f]) ? $nomsFilieres[$f] : $f;
...
=======
    ...
    function genererTabHeader($salles){
        $html = '<table cellpadding="2" cellspacing="0" style="width:100%; border-collapse:collapse;">
                    <thead >
                        <tr style="background-color: #4472c4; font-size: 7.5px;text-align:center; color:white; ">
                            <th style="width:19%;border: 0.5px solid #89a5d9;font-weight: bold;">Date</th>
                            <th style="width:19%;border: 0.5px solid #89a5d9;font-weight: bold;">Heure</th>
                            <th style="width:33%;border: 0.5px solid #89a5d9;font-weight: bold;">Matiére</th>
                            <th style="width:39%;border: 0.5px solid #89a5d9;font-weight: bold;">Respo coordination</th>
                            <th style="width:23%;border: 0.5px solid #89a5d9;font-weight: bold;">Salle</th>
        ...
        foreach ($salles as $salle => $info) {
            $html .= '<tr style="font-size:6px; ">
                        <td style="text-align:center;border: 0.5px solid #89a5d9;">' . htmlspecialchars($info['date']) . '</td>
                        <td style="border: 0.5px solid #89a5d9;">' . htmlspecialchars($info['heure']) . '</td>
                        <td style="border: 0.5px solid #89a5d9;">' . htmlspecialchars($info['matiere']) . '</td>
                        <td style="border: 0.5px solid #89a5d9;">' . htmlspecialchars($info['coord']) . '</td>
                        <td style="border: 0.5px solid #89a5d9;">' . htmlspecialchars($info['salle']) . '</td>
                      </tr>';
        }
